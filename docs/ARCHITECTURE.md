...
| 총 도구 수 | 398 tools |
| 서버 수 | 64 servers |
| 어댑터 수 | 48 adapters (Phase 14 표준화 완료) |
| Core 모듈 | 5 modules |
| Gateway | 1 (단일 진입점) |
| 프레임워크 | FastMCP 3.1.1 |
...
→ {"status": "ok", "servers": 64, "tools": 398}
```

**인증:**
- 인증 없음 (Open Access). Rate Limiting으로 남용 방지.
...
### 2-3. Adapter Layer — `adapters/*.py` (48개 파일)
...
**감사 범위:** 64개 서버 + 48개 어댑터 전수 역공학 분석  
**감사 기준:** import 횟수 0 = Dead Code 판정

| 컴포넌트 | 위치 | 상태 | 근거 |
|----------|------|------|------|
| `error_response()` | `core/responses.py` | **DEAD CODE** | 48개 어댑터 + 64개 서버 전체에서 import 0건 |
...
### Open Access
인증 없음. 누구나 접근 가능. Rate Limiting으로 남용 방지.
...
| Phase 13 | 문서 9개 정비, 어댑터 48/48 표준화 완료, 380+ tools |
| Phase 14 | DART/CCXT/SEC/ECOS/Macro 확장, FRED 추가, 398 tools 달성 |
...
- [TOOL_CATALOG.md](TOOL_CATALOG.md) — All 398 tools by domain and complexity
